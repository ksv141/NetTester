...
public:
    struct PortStats
    {
        ...
        timeval    ntPrevRecvTime;      // время поступления предыдущего пакета или начала окна (для вычисления скорости потока)
        quint32    ntWndRecvBytes;      // число принятых байт от начала окна
        quint32    ntWndPktCounter;     // число принятых пакетов от начала окна
        ...
        quint32    ntMaxJitterUs;       // максимальная вариация задержки от начала измерения (мкс)
