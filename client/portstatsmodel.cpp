...
QVariant PortStatsModel::data(const QModelIndex &index, int role) const
{
    ...
    if (role == Qt::DisplayRole)
    {
        ...
        switch(row)
        {
            ...
            case e_STAT_NT_LOSS_COUNT: return qint32(stats.ntlosscount() > 0 ? stats.ntlosscount() : 0);
            case e_STAT_NT_OUT_WND_COUNT: return qint32(stats.ntoutofwndcount() - stats.ntlosscount());
            case e_STAT_NT_LOSS_KOEFF: return (stats.ntlosskoeff() > 0 ? stats.ntlosskoeff()*100 : 0);
            case e_STAT_NT_OUT_WND_KOEFF: return (stats.ntoutofwndkoeff() - stats.ntlosskoeff())*100;
            case e_STAT_NT_MMO_LOSS: return (stats.ntmmolosskoeff() > 0 ? stats.ntmmolosskoeff()*100 : 0);
